...
import random
import time
#dictionaries for every non-legendary pokemon from Gen1 listing every stat. name being the Key
...
TYPE_MATRIX = {'grass':{'fire': 0.5, 'grass': 0.5, 'water': 2.0,'poison':0.5,'ground':2.0,'flying':0.5,'bug':0.5,'rock':2.0,'dragon':0.5},'fire':{'fire':0.5,'grass':2.0,'water':0.5,'ice':2.0,'bug':2.0,'rock':0.5,'dragon':0.5},'water':{'fire': 2.0, 'grass': 0.5, 'water': 0.5,'ground':2.0,'rock':2.0,'dragon':2.0},'bug':{'grass':2.0,'fire':0.5,'bug': 0.5,'poison': 2,'fighting':0.5},'normal':{'rock':0.5,'ghost':0},'poison':{'grass':2.0,'bug':2.0,'poison':0.5,'ground':0.5,'rock':0.5,'ghost':0.5},'electric':{'grass': 0.5,'water':2.0 ,'flying':0.5 ,'electric': 0.5,'ground':0 ,'dragon': 0.5},'ground':{'grass': 0.5,'fire':2.0 ,'flying':0,'bug': 0.5 ,'poison': 2.0,'electric':2.0 ,'ground': 2.0,'rock': 2},'flying':{'grass': 2.0,'electric': 0.5,'fighting': 2.0,'psychic':2.0 ,'rock':0.5},'fighting':{'bug': 0.5,'normal': 2.0,'poison': 0.5,'ground': 0.5,'psychic': 0.5,'rock': 2.0,'ghost': 0,'ice': 2.0},'psychic':{'poison': 2.0,'fighting': 2.0 ,'psychic': 0.5},'rock':{'flying': 2,'fire': 2.0,'bug': 2.0,'ground': 0.5,'fighting': 0.5,'ice': 2.0},'ghost':{'normal': 0,'psychic': 0,'ghost': 2.0},'ice':{'grass': 2.0,'water': 0.5,'flying': 2.0,'ground': 2.0,'ice': 0.5,'dragon': 2.0},'dragon':{'dragon':2.0}}
opponent_roster=[]
trainer_roster=[]
...
class pokemon(object):
    ...
    def attack(self,opponent):
        ...
        if opponent.type in TYPE_MATRIX[self.type]:
            damage = (random.randint(8,12)/10)*(TYPE_MATRIX[self.type][opponent.type]*int(self.base*24/opponent.base))
            opponent.hp = opponent.hp - damage
            if opponent.hp<0:
                opponent.hp=0
            print('{} attacked and did {} damage'.format(self.name,damage))
            if TYPE_MATRIX[self.type][opponent.type]==2.0:
                print('Its super effective!')
            elif TYPE_MATRIX[self.type][opponent.type]==0.5:
                print('Its not very effective...')
            else:
                print('The move had no effect.')
        else:
            damage = (random.randint(8,12)/10)*int(self.base*20/opponent.base)
            opponent.hp = opponent.hp - damage
            if opponent.hp<0:
                opponent.hp=0
            print('{} attacked and did {} damage'.format(self.name,damage))
...
def print_health():
    ...
    print('            {}s HP:{}/{}'.format(opponent_roster[p2][0].name,opponent_roster[p2][0].hp,opponent_roster[p2][0].starting_health))
    print('           |'+('*'*int(60*opponent_roster[p2][0].hp/opponent_roster[p2][0].starting_health))+(' '*(60-int(60*(opponent_roster[p2][0].hp/opponent_roster[p2][0].starting_health))))+'|')
    print('\n')
    print(' {}s HP:{}/{}'.format(trainer_roster[p1][0].name,trainer_roster[p1][0].hp,trainer_roster[p1][0].starting_health))
    print('|'+('*'*int(60*trainer_roster[p1][0].hp/trainer_roster[p1][0].starting_health))+(' '*(60-int(60*(trainer_roster[p1][0].hp/trainer_roster[p1][0].starting_health))))+'|')
...
def main():
    ...
    print('Junbi wa ii? Are you ready?')
    print('3')
    time.sleep(1)
    print('2')
    time.sleep(1)
